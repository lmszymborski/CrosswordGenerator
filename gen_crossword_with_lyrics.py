...
import requests
import sys
from gen_word_list_music import * 
...
def load_tracks(artist_name):
    token = authorize()
    tracks = get_artist_tracks(artist_name, token)
    track_words = []
    definitions = {}
    for track in tracks:
        words = track.split(' ')
        for word in words:
            if word.strip() not in track_words and len(word) > 2:
                new_word = ''
                for letter in word:
                    if (letter.isalnum()):
                        new_word += letter
                ...
                if definition is not None:
                    track_words.append(new_word.strip())
                    definitions[new_word] = definition
    print(track_words)
    return track_words, definitions
...
def repeated_generation(words, maxWords, iterations, threshold, sort_words):
    ...
    if result is None or worstResult is None:
        print("Unable to find a crossword over that threshold")
        result, saved_count, crossword_list = generate(words, maxWords, sort_words)
        worstResult = result
        saved_worst_count = saved_count
...
def question_to_word(crossword_list, definitions):
...
def main():
    if len(sys.argv) < 2:
        sys.exit("Artist name required")
    iterations = 1000
    artist = sys.argv[1]
    threshold = 0
    words, definitions = load_tracks(artist)
    maxWords = len(words)


    # sorted
    crossword, count, worst_crossword, worst_count, scores, crossword_list = repeated_generation(words, maxWords, iterations, threshold, True)
    print(crossword)
    ...
    clues = question_to_word(crossword_list, definitions)
